...
- [Risk, realized] `dtrpg-ui`'s executors are backed by a platform dispatcher (GCD on macOS), not a Tokio runtime, so an async `reqwest::get(...).await` inside `cx.spawn` has no reactor to resolve DNS/open sockets on and fails every time → Mitigation: use `reqwest::blocking::get` inside `background_executor().spawn(async move {...})`, matching the pattern `dtrpg-core`'s SDK gateway already uses (`tokio::runtime::Runtime::block_on` from background-executor threads). See tasks 8.1/8.2.
- [Risk, realized] On any install with a pre-existing catalog cache saved before `cover_url` mapping worked, every cached item has `cover_url: null`. The auto-load policy's "skip live fetch when cache is fresh and count matches" optimization (`catalog-auto-load-policy`) then serves that cover-url-less cache indefinitely with no live refetch, no error, and no visible sign thumbnails should be loading at all → Mitigation: `CacheMetadata` gained `schema_version`; a mismatch (including caches predating the field, which default to `0`) is now treated as stale regardless of age, forcing a one-time live refetch that repopulates `cover_url`. See tasks 9.1-9.3.
