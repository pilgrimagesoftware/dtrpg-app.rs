...
- [x] 1.2 Unit test: calling `log_alert` appends an entry to `alert_snapshot().entries` with the given label/message, without adding anything to `in_progress` or `recent`
...
- [x] 3.5 Manually open the Manage Collections dialog against a stub/error-mode
