...
- Change the value-font role's default from `Optima` to `Gotham`, dropping `Optima` from the curated list.
...
- `detail-panel-value-font` (a separate, sequenced proposal) depends on `LibriTheme.value_font` landing here — it consumes this field directly rather than introducing its own constant.
