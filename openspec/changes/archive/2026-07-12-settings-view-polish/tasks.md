...
- [x] 1.3 Run the app and visually confirm the label right-aligns and the value left-aligns without breaking the existing `items_baseline()` vertical alignment
...
- [x] 5.2 Manually verify: Account's Email/API Key row labels right-align and values left-align
- [x] 5.3 Manually verify: Downloads' "Change…" and reveal buttons show folder icons, still open the folder picker / reveal the storage location, and still show their tooltips on hover
- [x] 5.4 Manually verify: About shows a real (non-"unknown") git commit hash, a plausible build date, and the correct target triple for a normal `cargo build` in this git checkout
- [x] 5.5 Manually verify: About's rows are right-aligned with no visible borders
