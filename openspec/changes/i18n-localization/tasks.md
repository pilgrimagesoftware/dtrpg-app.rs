...
- [x] 1.1 Add `rust-i18n = "3"` to `[workspace.dependencies]` in the root `Cargo.toml`
- [x] 1.2 Add `sys-locale = "0.3"` to `[workspace.dependencies]` in the root `Cargo.toml`
- [x] 1.3 Add `rust-i18n` and `sys-locale` to `[dependencies]` in `crates/dtrpg-ui/Cargo.toml`

## 2. Locale Files

- [x] 2.1 Create `crates/dtrpg-ui/i18n/en.yaml` with all string keys (see key inventory below)
- [x] 2.2 Create `crates/dtrpg-ui/i18n/fr.yaml` as an English-value stub with a `# UNTRANSLATED STUB` header comment
- [x] 2.3 Create `crates/dtrpg-ui/i18n/de.yaml` as an English-value stub with a `# UNTRANSLATED STUB` header comment
...
- [x] 3.1 Create `crates/dtrpg-ui/src/i18n/mod.rs` with the `rust_i18n::i18n!("i18n")` macro invocation (points to `crates/dtrpg-ui/i18n/`) and an `pub fn init()` function that reads `sys_locale::get_locale()`, strips the region tag, and calls `rust_i18n::set_locale()` (defaulting to `"en"`)
- [x] 3.2 Declare `pub mod i18n;` in `crates/dtrpg-ui/src/lib.rs`
- [x] 3.3 Call `dtrpg_ui::i18n::init()` (or `crate::i18n::init()`) early in app startup, before any view renders -- place this in `util/init.rs` or the top of `ui/windows/app.rs`

## 4. Replace Strings in Views

- [x] 4.1 `sidebar_view.rs`: replace the 6 nav item label literals and the "Libri" wordmark with `t!()` calls
- [x] 4.2 `toolbar_view.rs`: replace sort menu labels, direction labels, group toggle label, view tab labels ("List", "Thumbs", "Grid"), sign-in/log-out menu item labels, and the `filter_title()` function's match arms
- [x] 4.3 `catalog_view.rs`: replace column name strings, empty-state messages, context menu item labels ("Download", "Remove Download"), and "Load Thumbnail" / platform reveal labels
- [x] 4.4 `detail_panel_view.rs`: replace button labels ("Read", "Download", "Downloaded", reveal label), tooltip text, `DescriptionItem` labels, and status strings ("On this device", "In the cloud")
- [x] 4.5 `settings_view.rs`: replace the "Settings" heading
- [x] 4.6 `settings_account_view.rs`: replace "Account", "Not signed in to DriveThruRPG", sign-in prompt, and "Log Out" label
- [x] 4.7 `settings_storage_view.rs`: replace "Catalog Storage Location" and the storage-missing warning
- [x] 4.8 `settings_file_openers_view.rs`: replace "File Openers", the description, and the empty state message
- [x] 4.9 `activity_panel_view.rs`: replace "Activity", "No recent activity.", and the empty-hint string

## 5. Build and Lint

- [x] 5.1 Run `cargo check --workspace` -- no errors
- [x] 5.2 Run `cargo clippy --all-targets --all-features -- -D warnings` -- no new warnings
- [x] 5.3 Run `cargo fmt --all` -- no formatting changes
- [x] 5.4 Run `cargo test --workspace` -- all tests pass (update any tests that assert on literal English strings)
