...
- [ ] 1.1 Add `is_available: bool` to `LibraryItem` (`crates/dtrpg-ui/src/data/library.rs`)
      with `#[serde(default = "default_true")]` and a `default_true() -> bool { true }`
      helper.
- [ ] 1.2 Set `is_available: true` in `LibraryItem::new` and any other in-crate constructors
      (stub/sample data generators) so existing call sites still compile.
- [ ] 1.3 Add a unit test in `library.rs` confirming a `LibraryItem` JSON blob without the
      `is_available` field deserializes with `is_available == true`.

## 2. Reconciliation logic

- [ ] 2.1 In `crates/dtrpg-ui/src/controllers/library.rs`, extract the reconcile-by-id merge
      ...
      Vec<LibraryItem>`.
- [ ] 2.2 Add unit tests for `reconcile_catalog` covering: item present in both (refreshed,
...
- [ ] 2.3 Update `set_catalog` (or split into `set_catalog` / a new reconciling entry point)
...
- [ ] 2.4 Update the fetch-completion call site (around the `ctrl.set_catalog(live_items,
...
- [ ] 3.1 Identify the catalog list/grid row rendering code and add a visual indicator
...
- [ ] 3.2 Confirm unavailable items are not excluded by any existing filter/search predicate
      (no code changes expected if none currently filter on availability — verify only).
- [ ] 3.3 Add a translation string for the unavailable indicator's tooltip/label under the
      existing i18n catalog, following `string-catalog` conventions.

## 4. Manual reload cooldown

- [ ] 4.1 Add `FORCE_RELOAD_COOLDOWN_SECS: u64 = 60` to
...
- [ ] 4.2 In `LibraryController::reload_catalog`, load `CacheMetadata` via the existing
...
- [ ] 4.3 Add unit/integration coverage for `reload_catalog`'s cooldown branch: reload
...
- [ ] 4.4 Confirm `clear_and_reload` (used after an on-disk cache clear) is unaffected by
...
- [ ] 5.1 Add `#[serde(skip)] availability_last_checked: Option<std::time::SystemTime>` to
      `LibraryItem`, alongside `thumbnail_last_attempted`.
- [ ] 5.2 Add `ITEM_CHECK_COOLDOWN_SECS: u64 = 300` to `constants.rs`.
- [ ] 5.3 Add `LibraryController::maybe_check_item(id: Arc<str>, cx: &mut Context<Self>)`:
...
- [ ] 5.4 Call `maybe_check_item` from `LibraryController::select_item` and from
      `TabsController::open_detail_tab`.
- [ ] 5.5 Add unit tests for `maybe_check_item`: cooldown suppresses a redundant check,
...
- [ ] 6.1 Add `checking_items: HashSet<Arc<str>>` to `LibraryController`; insert the id when
      a single-item check starts, remove it when the check completes, emitting
      `LibraryChanged` on both transitions.
- [ ] 6.2 Add `LibraryController::is_checking(&self, id: &str) -> bool`.
- [ ] 6.3 Render a checking indicator (spinner/overlay) on catalog entries where
...
- [ ] 7.1 Add `check_queue: VecDeque<Arc<str>>` to `LibraryController`.
- [ ] 7.2 Add `enqueue_checks(&mut self, ids: impl Iterator<Item = Arc<str>>, cx: &mut
      Context<Self>)`: pushes ids not already queued or in `checking_items` onto
      `check_queue`, then calls `drain_check_queue`.
- [ ] 7.3 Add `drain_check_queue(&mut self, cx: &mut Context<Self>)`, mirroring
...
- [ ] 7.4 Add unit tests for queue draining: single-flight (only one check in progress at a
...
- [ ] 8.1 Add `last_item_check_batch_secs: Option<u64>` to `CacheMetadata`
      (`crates/dtrpg-ui/src/data/catalog_cache.rs`) with `#[serde(default)]`.
- [ ] 8.2 Add `ITEM_CHECK_BATCH_COOLDOWN_SECS: u64 = 900` to `constants.rs`.
- [ ] 8.3 Add `LibraryController::request_check_batch(&mut self, cx: &mut Context<Self>)`:
...
- [ ] 8.4 Wire a manual trigger (menu action or catalog-view control) to call
      `request_check_batch`.
- [ ] 8.5 Add an automatic periodic trigger: a `cx.spawn` loop using
      `background_executor().timer(Duration)` between iterations that calls
      `request_check_batch` on each wake.
- [ ] 8.6 Add unit tests for `request_check_batch`: cooldown suppresses a request when a
...
- [ ] 9.1 Add `updated_date_after`-based partial fetch support: extend `LibraryService`
...
- [ ] 9.2 Implement `list_items_updated_since` on the SDK-backed service
      (`dtrpg-core/src/services/sdk.rs`), setting `LibraryItemsParams::updated_date_after`
      and paginating like `list_items_paged`.
- [ ] 9.3 In `start_load_inner`'s count-check branch, when `remote_count > cached_count`,
...
- [ ] 9.4 Merge a successful partial fetch's results additively (append new ids, refresh
      + set `is_available = true` for existing ids) without running the "mark absent items
      unavailable" sweep.
- [ ] 9.5 Add unit tests: `remote_count > cached_count` triggers a partial fetch and merges
...
- [ ] 10.1 `cargo test -p dtrpg-ui` and `cargo test -p dtrpg-core` pass, including the new
       reconciliation, deserialization-default, reload-cooldown, single-item-check,
       check-queue, check-batch-cooldown, and partial-fetch tests.
- [ ] 10.2 `cargo clippy --all-targets --all-features -- -D warnings` passes.
- [ ] 10.3 `cargo fmt --all -- --check` passes.
