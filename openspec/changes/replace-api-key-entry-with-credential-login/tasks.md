...
- [ ] 1.1 Replace the API key field in the Settings Account tab sign-in form
      with email and password fields
- [ ] 1.2 Update submit-button enablement logic for the two-field form
- [ ] 1.3 Pre-fill the email field when a stored entry has an email but an
      invalid or expired application key

## 2. SettingsController and login flow

- [ ] 2.1 Update `SettingsController` to call the SDK credential exchange
...
- [ ] 2.2 Update loading/disabled state handling to span both calls

## 3. Credential storage

- [ ] 3.1 Extend the stored credential payload (`Credential` /
      `KeyringCredentialStore`) to include account email alongside the
      application key
- [ ] 3.2 Update `KeyringCredentialStore` read path to tolerate legacy
      entries with no email field
- [ ] 3.3 Update tests in `crates/dtrpg-ui/src/credentials/store.rs` for the
