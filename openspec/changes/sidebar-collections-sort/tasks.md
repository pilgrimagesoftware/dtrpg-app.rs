...
- [x] 1.1 Add `created_at: i64` to `CollectionEntry` (`crates/dtrpg-ui/src/data/collection.rs`)
  with `#[serde(default)]` so pre-existing cache JSON without the field still deserializes
- [x] 1.2 Update `collections_sdk.rs`'s `list_collections` and `create_collection` mappings
  to populate `created_at` via `util::datetime::parse_rfc3339_to_epoch(&attrs.date_created)
  .unwrap_or(0)`
- [x] 1.3 Update the `CollectionsStubService` in `services/collections.rs` to populate a
  non-zero `created_at` on its seeded entries so stub/dev builds can exercise date sorting
- [x] 1.4 Update `collections_cache.rs` tests' `make_entry` helper to set `created_at`

## 2. Sort types and helper

- [x] 2.1 Add `CollectionSortMethod { Name, DateCreated, ItemCount }` enum to
...
- [x] 2.2 Add `sort_collections(entries: &mut [CollectionEntry], method: CollectionSortMethod,
...
- [x] 2.3 Unit tests in `util/sort.rs` for each `CollectionSortMethod` variant in both
...
- [x] 3.1 Add `collection_sort: Option<String>` and `collection_sort_direction:
  Option<String>` (or a small serializable enum) fields to `UiPrefsFile`
  (`data/ui_prefs.rs`)
- [x] 3.2 Add `UiPrefs::collection_sort() -> CollectionSortMethod` and
  `collection_sort_direction() -> SortDirection` accessors, defaulting to `Name` /
  `Ascending`
- [x] 3.3 Add `UiPrefs::save_collection_sort(method, direction)` following the
  `save_collections_open` pattern

## 4. Controller

- [x] 4.1 Add `collection_sort: CollectionSortMethod` and `collection_sort_direction:
  SortDirection` fields to `LibraryController`, initialized from `UiPrefs` on construction
- [x] 4.2 Add `set_collection_sort(&mut self, method: CollectionSortMethod, cx: &mut
...
- [x] 4.3 Add `set_collection_sort_direction(&mut self, direction: SortDirection, cx: &mut
  Context<Self>)` mirroring `set_collection_sort`
- [x] 4.4 In `apply_collections`, sort the incoming entries using the controller's current
...
- [x] 5.1 In `sidebar_view.rs`, add a sort control (icon `Button` with `.dropdown_menu(...)`,
  ...
  button
- [x] 5.2 Menu offers "Name", "Date Created", "Item Count" items (checked state reflects
...
- [x] 5.3 Hide or disable the sort control when `collection_search.open` is true, matching
  how the header suffix already swaps to a search input in that state

## 6. Localization

- [x] 6.1 Add `sidebar.collections_sort_name`, `sidebar.collections_sort_date_created`,
...
- [x] 7.1 Run `cargo check --workspace --all-targets`
- [x] 7.2 Run `cargo clippy --workspace --all-targets --all-features -- -D warnings`
- [x] 7.3 Run `cargo fmt --all -- --check` (ran via `cargo +nightly fmt`, per this repo's
  rustfmt.toml requiring nightly-only options)
