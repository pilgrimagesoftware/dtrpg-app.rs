...
- [x] 1.1 Replace `Button::new("avatar-btn")` authenticated branch in `render_avatar_button` with a circular `div` (30×30 px, `rounded_full()`, accent-color background)
- [x] 1.2 Center the user's initial letter inside the circular `div` as white `text_xs`
- [x] 1.3 Attach the logout dropdown to the new `div` element (matching the existing `Button` dropdown behavior)
- [x] 1.4 Add a tooltip ("Account") to the authenticated circular `div`
