# download-collections-symlinks Specification

## Purpose
Mirrors downloaded catalog files into a `collections/` directory tree, organized by the user's collections, alongside the existing publisher-organized `items/` layout — without duplicating file content on disk.
