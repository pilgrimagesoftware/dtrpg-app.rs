# download-disk-space-check

## Purpose

Before a download-queuing action commits files to the download queue, calculate the total size
of the not-yet-downloaded files it would enqueue and compare that against the free disk space at
the storage root. Warn the user with a confirmation dialog when space is insufficient, so a large
entry or a "Download All" action doesn't fill the disk mid-download without warning.
