# settings-appearance-page Specification
...
The Settings window SHALL provide an "Appearance" page, reachable from the settings sidebar alongside Account, Downloads Location, File Openers, Advanced, and About, presenting a picker for each font role (body, value, label, monospace), a "Text Scale" control, and a picker for the active color theme.

#### Scenario: Opening the Appearance page
- **WHEN** the user selects "Appearance" in the settings sidebar
- **THEN** the page shows the current body font, value font, label font, monospace font, text scale, and theme selections, each as a distinct picker or control

### Requirement: Appearance selections apply immediately
Changing any font role, the text scale, or the theme in the Appearance page SHALL apply the change to the running app immediately, without requiring a restart or window reopen.
...
The selected body font, value font, label font, monospace font, text scale, and theme SHALL persist across app restarts.
