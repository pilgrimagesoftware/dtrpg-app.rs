...
`DataTable` with `TableState::col_resizable(true)`, and each of the Name, Type, Size, and Status
columns SHALL be individually marked `resizable(true)`, so the user can drag a column divider to
resize that column.
