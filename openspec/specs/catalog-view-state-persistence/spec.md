# catalog-view-state-persistence Specification

## Purpose
Persist the catalog's sidebar filter, sort method, grouping, and presentation mode across app sessions, so users don't have to re-apply their preferences every launch.
