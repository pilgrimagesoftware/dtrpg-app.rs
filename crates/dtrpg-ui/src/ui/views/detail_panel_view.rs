...
fn render_item_tier(item: &LibraryItem, storage_root_path: &Path, entity: Entity<LibraryController>,
    ...
    div().flex()
         ...
         .child(match selected_file {
                    Some(file) => render_item_metadata(item, file, storage_root_path,
...
/// download location).
fn render_item_metadata(item: &LibraryItem, file: &LibraryItemFile, storage_root_path: &Path,
                        entity: Entity<LibraryController>, colors: &ColorTokens, cx: &App)
                        -> impl IntoElement + 'static {
    let name_value =
        copyable_value(SharedString::from(format!("file-name-{}", file.id)), file.name.to_string());
    ...
    div().flex()
         ...
         .gap(px(12.0))
         .child(metadata)
         .child(render_file_other_details(&item.id, file, item.status == ItemStatus::Downloaded,
                                          storage_root_path, entity, colors, cx))
}

/// Builds the shared toggle-state key for a single file's "Other details"
/// disclosure within a multi-item entry's item tier.
fn file_other_details_key(entry_id: &str, file_id: &str) -> Arc<str> {
    Arc::from(format!("{entry_id}:{file_id}"))
}
...
fn render_file_other_details(entry_id: &str, file: &LibraryItemFile, is_downloaded: bool,
                             storage_root_path: &Path, entity: Entity<LibraryController>,
                             colors: &ColorTokens, cx: &App)
                             -> impl IntoElement + 'static {
    let toggle_key = file_other_details_key(entry_id, &file.id);
