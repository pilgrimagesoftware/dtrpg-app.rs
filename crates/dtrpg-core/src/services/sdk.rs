...
impl HttpSdkLibraryGateway {
    ...
    /// tokens.
    fn from_keyring_with_tokens(tokens: dtrpg_ui::services::LoginTokens)
    ...
    fn build(application_key: String, access_token: String, refresh_token: String,
        ...
        sdk.apply_auth_response(AuthTokenResponse::new(access_token,
                                                       refresh_token,
                                                       refresh_token_ttl))
...
fn map_order_product(item: &OrderProductItem, publishers: &HashMap<u64, String>,
    ...
    // `relationships.product` against the response's sideloaded `included`
    // array rather than embedding it directly on `attributes` — fall back to
    // the embedded field in case a future/legacy response shape does embed it
    // inline.
    let product_info = item.relationships
                           .as_ref()
    ...
    let mut format_parts: Vec<String> = attributes.files
                                                  .iter()
